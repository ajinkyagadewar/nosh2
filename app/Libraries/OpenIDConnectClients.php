...
class OpenIDConnectClient {
    ...
    public function authenticate($uma = false, $type = '') {
        ...
        if (isset($_REQUEST["code"]) && isset($_SESSION['openid_connect_state'])) {
            ...
            if (property_exists($token_json, 'id_token')) {
                ...
                if (isset($token_json->patient))
                    $this->patientToken = $token_json->patient;
                return true;
            }
        } else {
            if ($uma == false) {
    ...
    private function verifyJWTclaimsRefresh($claims) {
        return (($claims->iss == $this->getProviderURL()) && (($claims->aud == $this->clientID) || (in_array($this->clientID, $claims->aud))));
    }
